...
#This file contains all final code produced for part 1 of this assignment.
#Basic data sorting and plotting are followed by ACF, PACF, and Differencing
#analysis, before the file concludes with an ARIMA model and related 
#analyses.
...
#Fitting an ARIMA model for TLB, for only 1960 to 2012 for later forecasting:
model_TLB <- ts_data %>%
  filter(Year <= 2012) %>%
  model(ARIMA(TLB))

#Fitting an ARIMA model for TFR, for only 1960 to 2012 for later forecasting:
model_TFR <- ts_data %>%
  filter(Year <= 2012) %>%
  model(ARIMA(TFR))

#Performing residual analysis on the above model: 

#Checking residual autocorrelation for TLB:
augment(model_TLB) %>%
  ACF(.resid) %>%
  autoplot()

#Performing a Portmenteau test to check for independence: 
augment(model_TLB) %>%
  features(.resid, portmanteau_tests)

#Repeating the above code for the TFR model:
augment(model_TFR) %>%
  ACF(.resid) %>%
  autoplot()
